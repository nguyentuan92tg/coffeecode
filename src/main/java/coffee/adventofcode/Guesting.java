...
public class Guesting {
    ...
    public Map<Character, Integer> result(List<Point> inputs){
        Character result;
        Map<Character, Integer> map = new HashMap<>();
        Points p = new Points(inputs);

        List<Point> points = new ArrayData().build(p.findMaxWidth(), p.findMaxWeight());

        for (int i = 0; i < points.size(); i++) {
             result = find(inputs, points.get(i), inputs);
             if(result != null ){
    ...
    public Character find(List<Point> points, Point currentPoint, List<Point> inputs){
